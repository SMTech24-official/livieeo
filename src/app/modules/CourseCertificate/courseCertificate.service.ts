...
const createCourseCertificateIntoDB = async (
  payload: CourseCertificate,
  userJwt: JwtPayload
) => {
  ...
  const certificateNo = `CERT-${dayjs().format("YYYY")}-${String(seq).padStart(
    6,
    "0"
  )}`;
  ...
  const upload: any = await fileUploader.uploadPdfBuffer(
...
const getMyCertificatesFromDB = async (
